...
const machineAwardInfo = {
  autonomous: {
    ...
    sample: 'Our most reliable auto uses vision and gyro feedback to align before scoring. We tested it from multiple starting positions so it still works after a slightly imperfect setup.'
  },
  creativity: {
    ...
    sample: 'Most teams solved this with a straight arm, but we used a folding linkage so the mechanism stays protected inside the frame until needed.'
  },
  engineering: {
    ...
    sample: 'The shooter, indexing sensor, and driver feedback were designed together so the driver knows when the game piece is staged and the code can maintain a consistent firing sequence.'
  },
  industrial: {
    ...
    sample: 'We kept the electronics panel visible and serviceable, routed wires through protected paths, and placed high-repair parts where pit students can reach them quickly.'
  },
  control: {
    ...
    sample: 'Our driver presses one button and the robot uses vision plus gyro heading to align. The driver still controls final movement, but the assist reduces overcorrection.'
  },
  quality: {
    title: 'Quality Award',
    bullets: [
    ...
    sample: 'The intake takes the most hits, so we made it flexible, labeled replacement parts, and keep a spare preassembled. We can swap the front roller in a few minutes.'
  }
};
...
const teamAwardInfo = {
  ei: {
    ...
    sample: 'We support younger FIRST students through FLL and FTC mentoring, which creates a pathway from elementary robotics to high school engineering.'
  },
  gp: {
    ...
    sample: 'When another team had CAN issues, two students helped trace their bus and shared our checklist. We compete hard, but we want every team to get on the field.'
  },
  imagery: {
    title: 'Imagery Award',
    bullets: [
    ...
    sample: 'Our CyBear identity shows up in our pit, robot labels, team shirts, handouts, and web materials so students and visitors recognize us immediately.'
  },
  judges: {
    title: 'Judges Award',
    bullets: [
    ...
    sample: 'Our unique story is how new students rebuilt team systems after turnover: training checklists, pit roles, and documentation made the team stronger for future years.'
  },
  rising: {
    ...
    sample: 'We had many new students, so we built training groups for CAD, fabrication, programming, and pit work. The goal was to make the team stronger next year, not just finish this robot.'
  },
  spirit: {
    ...
    sample: 'Our spirit is not only cheering. We use team roles so everyone has a match-day purpose, and we celebrate pit fixes, scouting, media, and drive team equally.'
  },
  sustainability: {
    ...
    sample: 'For people, we train new students with modules. For prosperity, we track fundraising and sponsor communication. For planet, we reuse materials and plan purchases to reduce waste.'
  }
};
...
document.querySelectorAll('.machine-awards .award-chip').forEach(button => {
  button.addEventListener('click', () => {
    document.querySelectorAll('.machine-awards .award-chip').forEach(b => b.classList.remove('active'));
    button.classList.add('active');
    renderAward(document.getElementById('machineAwardOutput'), machineAwardInfo[button.dataset.award]);
  });
});

document.querySelectorAll('.team-awards .award-chip').forEach(button => {
  button.addEventListener('click', () => {
    document.querySelectorAll('.team-awards .award-chip').forEach(b => b.classList.remove('active'));
    button.classList.add('active');
    renderAward(document.getElementById('teamAwardOutput'), teamAwardInfo[button.dataset.award]);
  });
});
...
function allowDrop(area) {
  ...
  area.addEventListener('drop', event => {
    ...
    if (area.classList.contains('judge-sort-bank')) area.appendChild(draggedChip);
    else {
...
const sortFeedback = document.querySelector('.judge-sort-slide .activity-feedback');
...
if (checkSort) {
  checkSort.addEventListener('click', () => {
    ...
    if (sortFeedback) sortFeedback.textContent = `${correct} of ${total} sorted correctly.`;
  });
}
...
document.querySelectorAll('.response-card').forEach(card => {
  card.addEventListener('click', () => {
    document.querySelectorAll('.response-card').forEach(c => c.classList.remove('correct', 'incorrect'));
    const isCorrect = card.dataset.correct === 'true';
    card.classList.add(isCorrect ? 'correct' : 'incorrect');
    const feedback = document.querySelector('.response-feedback');
    if (feedback) feedback.textContent = isCorrect ? 'Correct. It is specific, evidence-based, and student-owned.' : 'Try again. Look for the answer with a clear example and result.';
  });
});
...
Reveal.on('slidechanged', event => {
  if (event.currentSlide && event.currentSlide.id === 'complete' && !quizPassed) {
    ...
    setTimeout(() => {
      ...
      if (hint) hint.textContent = `Passing score: ${passingScore}/${Object.keys(judgingCorrectAnswers).length}.`;
    }, 0);
  }
});
