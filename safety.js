  ...
  document.querySelectorAll(slideSelector).forEach(slide => {
    ...
    if (checkButton) {
      checkButton.addEventListener('click', () => {
        ...
        chips.forEach(chip => {
          ...
          if (!zone) { allPlaced = false; allCorrect = false; chip.classList.add('incorrect'); return; }
          if (zone.dataset.zone === chip.dataset.answer) chip.classList.add('correct');
          else { chip.classList.add('incorrect'); allCorrect = false; }
        });
        if (feedback) {
          if (!allPlaced) { feedback.textContent = 'Place every card before checking.'; feedback.style.color = '#b00020'; }
          else if (allCorrect) { feedback.textContent = 'Correct. Safe choices reduce injuries and robot failures.'; feedback.style.color = '#2e7d32'; }
          else { feedback.textContent = 'Some cards need to move. Try again.'; feedback.style.color = '#b00020'; }
        }
      });
    }
    ...
    if (resetButton) {
      resetButton.addEventListener('click', () => {
        startingOrder.forEach(chip => { chip.classList.remove('correct', 'incorrect'); bank.appendChild(chip); });
        if (feedback) feedback.textContent = '';
      });
    }
  });
...
function initOrderSort() {
  document.querySelectorAll('.safety-order-slide').forEach(slide => {
    ...
    function getDragAfterElement(container, y) {
      const draggableElements = Array.from(container.querySelectorAll('.order-card:not(.dragging)'));
    ...
    if (checkButton) {
      checkButton.addEventListener('click', () => {
        let correct = true;
        Array.from(list.querySelectorAll('.order-card')).forEach((card, index) => {
          card.classList.remove('correct', 'incorrect');
          if (Number(card.dataset.order) === index + 1) card.classList.add('correct');
          else { card.classList.add('incorrect'); correct = false; }
        });
        if (feedback) {
          if (correct) { feedback.textContent = 'Correct. This is a safe sequence.'; feedback.style.color = '#2e7d32'; }
          else { feedback.textContent = 'Not quite. Move the red cards and try again.'; feedback.style.color = '#b00020'; }
        }
      });
    }
    ...
    if (resetButton) {
      resetButton.addEventListener('click', () => {
        startingOrder.forEach(card => { card.classList.remove('correct', 'incorrect'); list.appendChild(card); });
        if (feedback) feedback.textContent = '';
      });
    }
  });
...
function initReviewChecklist() {
  document.querySelectorAll('.review-grid').forEach(grid => {
    ...
    function update() {
      const count = items.filter(item => item.classList.contains('reviewed')).length;
      if (feedback) feedback.textContent = `${count} of ${items.length} reviewed.`;
    }
    items.forEach(item => {
      item.addEventListener('click', () => { item.classList.toggle('reviewed'); update(); });
    });
    update();
  });
}
...
Reveal.on('slidechanged', event => {
  if (event.currentSlide && event.currentSlide.id === 'complete' && !quizPassed) {
    ...
    setTimeout(() => {
      ...
      if (hint) hint.textContent = `Passing score: ${passingScore}/${Object.keys(safetyCorrectAnswers).length}.`;
    }, 0);
  }
});
