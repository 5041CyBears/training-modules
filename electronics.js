...
Reveal.on("slidechanged", (event) => {
  if (
    ...
    setTimeout(() => {
      ...
      document.getElementById("quizHint").textContent =
        `Passing score: ${passingScore}/${Object.keys(electronicsCorrectAnswers).length}.`;
    }, 0);
  }
});
    ...
    if (checkButton) {
      checkButton.addEventListener("click", () => {
        ...
        if (!allPlaced) {
          ...
          feedback.textContent =
            "Correct. Good wiring choices reduce failures.";
...
function initOrderSort() {
  document.querySelectorAll(".trouble-order-slide").forEach((slide) => {
    ...
    checkButton.addEventListener("click", () => {
      ...
      if (correct) {
        feedback.textContent =
          "Correct. This is a good first-pass troubleshooting order.";
        feedback.style.color = "#2e7d32";
      } else {
